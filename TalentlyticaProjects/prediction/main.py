...
def segment_extract_data(data,  path_to_write, reader=None, singleFile=True):
    ...
    for i in documents:
        ...
        for j in range(len(images)):
            ...
            images[j].save(path_to_write+fname.split('.')
                           [0]+'_' + str(j) + '.jpg', 'JPEG')
            resumes_img.append(path_to_write+fname.split('.')
                               [0]+'_' + str(j) + '.jpg')
        ...
        for i in resumes_img:
            ...
            output_img, label, dilate, c_dict, df1, split_img = ts.get_text_seg(
                frame, img)
            cv2.imwrite(path_to_write+img.split('.')[0]+".png", output_img)
            for i in range(len(split_img)):
                cv2.imwrite(path_to_write+img.split('.')
                            [0]+str(i)+".png", split_img[i])
...
def extract_data(filePath, skills, nlp, temp_path, reader=None):
    ...
    textList, fileName, fullText = segment_extract_data(
        filePath, temp_path, reader)
...
def batch_extract_data(filePath, skills, nlp, temp_path):
    ...
    for file in os.listdir(filePath):
        data = extract_data('{}/{}'.format(filePath, file),
                            skills, nlp, temp_path)
...
def predict(filepath, uid=None):

    # Adding skills database
    skills = add_skills_data('/home/c2284f2446/list_of_skills.txt')
    skills[0] = '.NET'  # First skills is not UTF-8 so we need to replace it

    temp_path = ("./segmentation-{}/".format(uid))
    ...
    nlp = spacy.load(
        '/home/c2284f2446/new/TalentlyticaProjects/model/model_exp')
